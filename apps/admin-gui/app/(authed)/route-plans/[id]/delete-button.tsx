...
export function DeleteRoutePlanButton({
  id,
  name,
}: {
  id: string;
  ...
  async function handleDelete() {
    setDeleting(true);
    setError(null);
    const res = await fetch(`/api/route-plans/${id}`, { method: 'DELETE' });
    if (!res.ok) {
  ...
  return (
    <div className="border border-error/50 bg-error/10 rounded p-4 text-sm">
      <p className="text-error mb-3">
        Delete route plan <span className="font-mono">{name}</span>? Campaigns
        referencing this plan will fail to dial until reconfigured.
      </p>
