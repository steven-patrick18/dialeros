...
export function EditCarrierForm({ carrier }: { carrier: CarrierState }) {
  const router = useRouter();
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [authMode, setAuthMode] = useState<AuthMode>(
    carrier.auth_mode as AuthMode,
  );
  const [codecs, setCodecs] = useState<Set<Codec>>(new Set(carrier.codecs));
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const password = String(fd.get('digest_password') ?? '');
    const body = {
      ...
      digest_username:
        authMode === 'digest'
          ...
          : undefined,
      // Only send password if non-empty so the server keeps the existing one.
      digest_password:
        authMode === 'digest' && password.length > 0 ? password : undefined,
      ip_acl:
        authMode === 'ip-acl' ? String(fd.get('ip_acl') ?? '') : undefined,
      codecs: orderedCodecs,
      max_channels: Number(fd.get('max_channels') ?? carrier.max_channels),
      max_cps: Number(fd.get('max_cps') ?? carrier.max_cps),
      mos_threshold: Number(fd.get('mos_threshold') ?? carrier.mos_threshold),
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-xl space-y-5">
      ...
      <Section title="Connection">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <Field label="Transport">
            <select
              name="transport"
              defaultValue={carrier.transport}
              className="input"
            >
              {TRANSPORTS.map((t) => (
      ...
      <Section title="Authentication">
          ...
          <>
            <Field label="Digest Username">
              <input
                ...
                required
                defaultValue={carrier.digest_username ?? ''}
                className="input"
                autoComplete="off"
              />
            </Field>
            ...
            >
              <input
                ...
                className="input"
                placeholder={
                  carrier.has_digest_password ? '●●●●●●●● (unchanged)' : ''
                }
        ...
        {authMode === 'ip-acl' && (
          <Field
            ...
            hint="Comma-separated. IPs or CIDR blocks."
          >
            <input
              name="ip_acl"
              required
              defaultValue={carrier.ip_acl ?? ''}
              className="input"
              autoComplete="off"
            />
          </Field>
        ...
        <div className="grid grid-cols-3 gap-4">
          <Field label="Max channels">
            <input
              name="max_channels"
              type="number"
              defaultValue={carrier.max_channels}
              min={1}
              max={10000}
              className="input"
            />
          ...
          <Field label="Max CPS">
            <input
              name="max_cps"
              type="number"
              defaultValue={carrier.max_cps}
              min={1}
              max={1000}
              className="input"
            />
          </Field>
          <Field label="MOS threshold">
            <input
              name="mos_threshold"
              type="number"
              step="0.1"
              defaultValue={carrier.mos_threshold}
              min={0}
              max={5}
              className="input"
            />
      ...
      <label className="flex items-center gap-2 text-sm">
        <input name="enabled" type="checkbox" defaultChecked={carrier.enabled} />
        <span>Enabled</span>
      </label>
